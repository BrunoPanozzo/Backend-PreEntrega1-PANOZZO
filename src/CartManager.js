...
class CartManager {
    ...
    //devolver todo el arreglo de carritos leidos a partir de un archivo de productos
    getCarts = async () => {
        try {
    ...
    //agregar, si sus campos de datos son válidos, un carrito al arreglo de carritos inicial y al archivo correspondiente
    addCart = async (products) => {
        //si llego a este punto, ya están validados los datos, puedo construir el objeto "cart"
        const cart = {
            id: this.#getNuevoID(),
            products
        }
    ...
    addProductToCart = async (cartId, prodId, quantity) => {
        ...
        const productosDeUnCarrito = this.#carts[cartIndex].products
        const productIndex = productosDeUnCarrito.findIndex(item => item.id === prodId)
        if (productIndex != -1) {
            productosDeUnCarrito[productIndex].quantity += quantity
        }
        else {
            const newProduct = {
                id: prodId,
            ...
            productosDeUnCarrito.push(newProduct)
        }
        await this.#updateCartsFile()
    }
}
